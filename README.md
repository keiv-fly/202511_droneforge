...
- `docs/`: static assets (`index.html`, `main.js`, `style.css`, wasm output later under `docs/`)
...
`cp target/wasm32-unknown-unknown/release/droneforge-web.wasm docs/droneforge-web.wasm`

4) Serve the `docs/` folder and open in browser:  
`cd docs; simple-http-server .`  
Then open the printed URL (e.g., `http://127.0.0.1:8000/`) to see the black canvas (and tick text).  
(`docs/index.html` already references the vendored `mq_js_bundle.js` from macroquad’s docs.)
